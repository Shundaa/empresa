...
public class QuizController {
    ...
    public ResponseEntity<List<QuizDao>> getAllLessons() {
        return ResponseEntity.ok(quizService.get());
    }

    @PostMapping("/add")
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<QuizDao> addNewLesson(@RequestBody QuizRequest request) {
        return ResponseEntity.ok(quizService.newLesson(request));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<QuizDao> addNewLesson(@PathVariable Long id, @RequestBody QuizRequest request) {
        return ResponseEntity.ok(quizService.edit(id, request));
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<QuizDao> getOneLesson(@PathVariable Long id) {
    ...
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<QuizDao> addNewLesson(@PathVariable Long id,
